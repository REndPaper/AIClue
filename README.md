...
사건 선택과 브리핑을 거쳐 용의자를 심문하고 3×3 공간에서 단서를 수집합니다. 범인과 흉기를 제출하면 게임 로직이 점수와 등급을 계산하고, 교수 역할의 AI가 수사 결과에 대한 코멘트를 생성합니다.
...
| 네이티브 라이브러리 로딩 오류 | OS에 맞는 Vulkan 백엔드, 패키지 복원, 그래픽 드라이버 |
...
- 프레임 시간·추론 지연·메모리 사용량의 정량 측정은 아직 진행하지 않았습니다.
